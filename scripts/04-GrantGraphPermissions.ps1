...
foreach ($permissionName in @($config.graphApplicationPermissions)) {
    ...
    $existingAssignments = Get-GraphCollectionAll -Uri "/v1.0/servicePrincipals/$($sp.id)/appRoleAssignments"
    if ($existingAssignments | Where-Object { $_.appRoleId -eq $permissionMap[$permissionName] }) {
...
Write-Host "Graph application permission grant step complete."
